...
def dashboard():
    ...
    if role == 'admin':
        staff_info = fetch_user_data(user_id)
        return render_template('dashboard.html', staff_info=staff_info, parking_entries=parking_entries_list, total_id=total_paid, total_available_id=total_non_occupied, total_parking_slots=total_parking_slots)
    elif role == 'cashier':
        staff_info = fetch_user_data(user_id)
        return render_template('dashboardCashier.html', staff_info=staff_info, parking_entries=parking_entries_list, total_id=total_paid, total_available_id=total_non_occupied, total_parking_slots=total_parking_slots)
    else:
        return redirect(url_for('login_page_redirect'))
...
def parking_entry():
    ...
    roi_text_response = roi_text()
    roi_text_data = roi_text_response.get_json()
    license_plate = roi_text_data.get('roiText', '')
    customer_name = roi_text_data.get('customerName', 'Guest')  # Get customer name from ROI text

    return render_template('parking_entry.html', staff_info=staff_info, transaction_id=transaction_id, entry_date=entry_date, entry_time=entry_time)
...
def parking_exit():
    ...
    roi_text_data = roi_text_response.get_json()
    license_plate = roi_text_data.get('roiText', '')
    customer_name = roi_text_data.get('customerName', 'Guest')
    ...
    if latest_entry_transaction:
        ...
        return render_template('parking_exit.html', staff_info=staff_info, exit_date=exit_date, exit_time=exit_time,
                               ...
                               duration=duration_formatted, parking_type=parking_type, total_fee=total_fee)
    else:
        return render_template('parking_exit.html', staff_info=staff_info, exit_date=exit_date, exit_time=exit_time)
...
def entry_submit():
    if request.method == 'POST':
        ...
        return render_template('parking_entry.html')
...
def exit_submit():
    ...
    if request.method == 'POST':
        ...
        return render_template('parking_exit.html', staff_info=staff_info, duration=duration_hours, parking_type=parking_type, total_fee=total_fee)
    return redirect(url_for('index'))
...
def map():
    ...
    return render_template('map.html', staff_info=staff_info)
...
def parking_activity():
    ...
    # Render the template with staff information
    return render_template('activity.html', staff_info=staff_info)
...
def history_transactions():
    ...
    # Render the template with staff information
    return render_template('history.html', staff_info=staff_info, parking_entries=parking_entries_list)
...
def staff_users():
    ...
    return render_template('staff_users.html', staff_accounts=staff_accounts, staff_info=staff_info)
...
def archives():
    ...
    return render_template('archive.html', archived_staff_accounts=archived_staff_accounts, staff_info=staff_info)
...
def logs_page():
    ...
    return render_template('logs.html', staff_info=staff_info, logs=logs, staff_name=staff_name, logs_pagination=logs_pagination)
